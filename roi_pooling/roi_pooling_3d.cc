...
class RoiPooling3DOp : public OpKernel {
    private:
        int pool_height_, pool_width_, pool_depth_;
    public:
        explicit RoiPooling3DOp(OpKernelConstruction* context) : OpKernel(context) {
                 ...
                 OP_REQUIRES_OK(context,
		   context->GetAttr("pool_depth", &pool_depth_));
        }
	    ...
	    int depth  = input_shape.dim_size(3);
            int channels = input_shape.dim_size(4);
		  ...
		  static_cast<int64>(pool_depth_)});
            ...
            RoiPooling3DKernelLauncher(input.data(), rois.data(),
                n_rois, channels,
		height, width, depth,
		pool_height_, pool_width_, pool_depth_,
                output.data(), argmax_output.data());
        }
};

REGISTER_KERNEL_BUILDER(Name("RoiPooling3D").Device(DEVICE_GPU), RoiPooling3DOp);

///////////// RoiPooling3DGrad


REGISTER_OP("RoiPooling3DGrad")
...
class RoiPooling3DGradOp : public OpKernel {
    private:
        int pool_height_, pool_width_, pool_depth_;
    public:
        explicit RoiPooling3DGradOp(OpKernelConstruction* context) : OpKernel(context) {
                 OP_REQUIRES_OK(context,
                   context->GetAttr("pool_height", &pool_height_));
        ...
        void Compute(OpKernelContext* context) override {
            ...
            int depth = orig_input_shape.dim_size(3);
            int channels = orig_input_shape.dim_size(4);
            ...
            RoiPooling3DGradKernelLauncher(orig_input.data(), orig_rois.data(),
		mb_size, n_rois, channels, height, width, depth, pool_height_, pool_width_, pool_depth_,
                orig_output.data(), orig_argmax_output.data(), orig_output_grad.data(), output.data());
        }
};


REGISTER_KERNEL_BUILDER(Name("RoiPooling3DGrad").Device(DEVICE_GPU), RoiPooling3DGradOp);
